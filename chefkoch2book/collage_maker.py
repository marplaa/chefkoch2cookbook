...
def make_collage(images_list, width, init_height, max_height):
    """
    Make a collage image with a width equal to `width` from `images`(list of urls) and save to `filename`.
    """
    ...
    while True:
        ...
        x = 0
        height = 0
        while images_list and height < max_height:
            ...
            if x > width:
                ...
                images_line = []
                x = 0
                height += last_image_height
            x += img.size[0] + margin_size
            images_line.append(img_path)
            last_image_height = img.size[1]
